...
def add_icon(slide, icon_path, left, top, width, label=None):
    """
    Adds an AWS icon and optional label text below it.
    Converts unsupported or .webp images to PNG automatically.
    """
    ext = os.path.splitext(icon_path)[1].lower()
    safe_path = icon_path

    # Convert WebP or any unsupported image format to PNG
    try:
        with Image.open(icon_path) as im:
            if im.format not in ["BMP", "GIF", "JPEG", "PNG", "TIFF", "WMF"]:
                png_path = os.path.splitext(icon_path)[0] + "_converted.png"
                im.save(png_path, format="PNG")
                safe_path = png_path
    except Exception as e:
        print(f"⚠️  Could not read {icon_path}: {e}")

    # Add the picture
    pic = slide.shapes.add_picture(safe_path, Inches(left), Inches(top), width=Inches(width))
